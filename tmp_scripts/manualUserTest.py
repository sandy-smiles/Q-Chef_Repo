...
def main():
  ...
  try:
    ...
    for ic_key in response_data.keys():
      print("""Familiarity Question...
      2) I know how this tastes.
      1) I have some idea how this tastes.
      0) I don't really know how this tastes.
      """, end='')
      ic_fam_rating = int(input(f"Familiarity of {ic_data[str(ic_key)]['name']}: "))
      request_data['familiarity_ratings'][ic_key] = min(max(0, ic_fam_rating), 2)
      print("""Taste Question...
      2) I like to eat this
      1) I sometimes like to eat this.
      0) I don't like to eat this.
      """, end='')
      ic_tas_rating = int(input(f"Taste of {ic_data[str(ic_key)]['name']}: "))
      request_data['taste_ratings'][ic_key] = min(max(0, ic_tas_rating), 2)
      print()
  ...
  try:
    ...
    for r_key in response_data.keys():
      ...
      0) I don't really know how this tastes.
      """, end='')
      r_fam_rating = int(input(f"Familiarity of {response_data[r_key]['title']}: "))
      request_data['familiarity_ratings'][r_key] = min(max(0, r_fam_rating), 2)
      print("""Taste Question...
      2) I like to eat this.
      1) I sometimes like to eat this.
      0) I don't like to eat this.
      """, end='')
      r_tas_rating = int(input(f"Taste of {response_data[r_key]['title']}: "))
      request_data['taste_ratings'][r_key] = min(max(0, r_tas_rating), 2)
      print("""Surprise Question...
      2) I think these ingredients often appear in recipes together.
      1) I think these ingredients sometimes appear together, maybe?
      0) I think these ingredients are rarely, if ever, seen together.
      """, end='')
      r_sur_rating = int(input(f"Surpise of {response_data[r_key]['title']}: "))
      request_data['surprise_ratings'][r_key] = min(max(0, r_sur_rating), 2)
      print()
    with open(save_folder+url_path+f'-{user_name}.json', 'a') as f:
      f.write('')
    with open(save_folder+url_path+f'-{user_name}.json', 'w') as f:
