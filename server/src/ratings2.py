...
# - Output:
#   - (string) error
def updateSingleIngredientRating(user_id, ingredient_id, ratings, rating_types):
  ...
  if err:
    err = f'[updateSingleIngredientRating - ERROR]: Unable to update user document with ratings for ingredient {ingredient_id}, err: {err}'
    debug(err)
    return err
  return ''
...
def updateIngredientRatings(data, rating_types):
  ...
  for ingredient_id in ingredient_ids.keys():
    ...
    err = updateSingleIngredientRating(user_id, ingredient_id, ratings, rating_types)
    if err:
...
# - Output:
#   - (string) error
def updateSingleRecipeRating(user_id, recipe_id, ratings, rating_types):
  rating_type = rating_type.lower()
  ...
  err = updateDocument('users', user_id, updating_data)
  if err:
    err = f'[updateSingleRecipeRating - ERROR]: Unable to update user document with ratings for recipe {recipe_id}, err: {err}'
    debug(err)
    return err
  return''
...
def updateRecipeRatings(data, rating_types):
  ...
  user_id = data['userID']
  recipe_ids = data[rating_types[0]]
  for recipe_id in recipe_ids.keys():
    ratings = {}
    for rating_type in rating_types:
      ratings[rating_type] = data[rating_type+'_ratings'][recipe_id]
    err = updateSingleRecipeRating(user_id, recipe_id, ratings, rating_types)
    if err:
